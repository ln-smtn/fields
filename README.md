...
## Устройство инференса (GPU / MPS / CPU)

Инференс (`ftw inference run`) определяется автоматически в функции `ftw_inference_device_args` в `ftw_oneshot.py`:

| Платформа | Что используется | Аргумент для ftw |
|-----------|-----------------|------------------|
| **macOS Apple Silicon** (M1/M2/M3/M4) | Metal Performance Shaders (MPS) | `--mps_mode` |
| **Linux + NVIDIA GPU** | CUDA, устройство 0 | `--gpu 0` |
| **Нет GPU / MPS недоступен** | CPU (медленнее) | `--gpu -1` |

Выбранное устройство записывается в `manifest.json` → поля `inference_device` и `inference_device_note`.

Если нужно **принудительно переключить** устройство — отредактируйте вызов `ftw_inference_device_args` в `ftw_oneshot.py` (строка с `_ftw_dev_args, _ftw_dev_human, _ftw_dev_code = ftw_inference_device_args(...)`), или передайте аргументы напрямую через `ftw inference run --help`.
